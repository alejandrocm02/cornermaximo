...
export async function SiteFooter() {
  ...
  return (
    <footer className="relative mt-16 border-t border-pitch-border/70">
      {/* Resplandor superior: cierra la página sin una línea dura. */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 -top-px h-px bg-grad-brand opacity-30"
      />
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-8 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
          ...
          <section aria-label="Fuente de los datos" className="fs-panel p-5">
            <div className="flex items-center gap-2.5">
              <span
                aria-hidden="true"
                className="grid h-7 w-7 place-items-center rounded-lg bg-grad-brand font-display text-2xs font-bold text-black"
              >
                F
              </span>
            ...
            <p className="mt-3 text-sm text-pitch-muted">
              ...
              <a
                ...
                target="_blank"
                className="rounded font-medium text-pitch-accent hover:underline"
              >
                API-Football
              </a>
            ...
            <ul className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              {SECTIONS.map((s) => (
                <li key={s.href}>
                  <Link
                    href={s.href}
                    className="block rounded py-1.5 text-pitch-muted transition-colors hover:text-white"
                  >
                    {s.label}
                  </Link>
                </li>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <nav aria-label="Enlaces del pie" className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-pitch-muted">
            {FOOTER_LINKS.map((l) => (
              <Link key={l.href} href={l.href} className="rounded py-1 transition-colors hover:text-white">
                {l.label}
              </Link>
            ))}
          </nav>
