...
const STATUS_CLASS: Record<string, string> = {
  ...
  FINISHED: 'border-pitch-border bg-pitch-elevated/70 text-pitch-muted',
  SCHEDULED: 'border-pitch-accent/25 bg-pitch-accent/10 text-pitch-accent',
...
function competitionLogo(match: MatchListItem): string {
  return match.competition.logoUrl ?? `https://media.api-sports.io/football/leagues/${match.competition.externalId}.png`;
}
...
function TeamRow({ team, side, showScore }: { team: MatchListTeam | null; side: 'home' | 'away'; showScore: boolean }) {
  return (
    <div className="grid grid-cols-[2.25rem_minmax(0,1fr)_3rem] items-center gap-2.5">
      ...
      <span className="text-right font-display text-xl font-bold tabular-nums text-white">{showScore ? team?.goals ?? '—' : ''}</span>
    </div>
  );
}
...
export function MatchCard({ match }: { match: MatchListItem }) {
  ...
  return (
    <article className={`fs-panel-interactive group overflow-hidden ${live ? '!border-pitch-danger/30 shadow-[0_0_35px_-25px_rgba(255,62,82,.65)]' : ''}`}>
      ...
      <Link href={`/partidos/${match.id}`} className="block space-y-3 px-4 py-4">
        ...
        {match.hasPenalties && finished && (match.home?.penaltyGoals != null || match.away?.penaltyGoals != null) && (
          <p className="text-right text-[10px] text-pitch-muted">Penaltis {match.home?.penaltyGoals ?? '—'}–{match.away?.penaltyGoals ?? '—'}</p>
        )}
      </Link>

      <div className="flex items-center justify-between gap-3 border-t border-pitch-border/60 bg-black/10 px-4 py-3 text-xs">
        <time dateTime={match.kickoffAt} className="text-pitch-muted">{kickoff.toLocaleString('es-ES', { timeZone: 'Europe/Madrid', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</time>
        <Link href={`/partidos/${match.id}`} className="inline-flex items-center gap-1.5 font-semibold text-pitch-accent">Match Center <span aria-hidden="true" className="transition-transform group-hover:translate-x-1">→</span></Link>
      </div>
    </article>
  );
}
