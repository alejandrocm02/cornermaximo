...
export function ShareComparisonButton({ p1, p2 }: { p1: string; p2: string }) {
  ...
  async function share() {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Comparación FutStats', text: 'Mira esta comparación en FutStats', url: comparisonUrl });
        return;
      }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <a href={imageUrl} download={`futstats-${p1}-vs-${p2}.png`} className="fs-btn-ghost inline-flex">
        Guardar PNG
      </a>
