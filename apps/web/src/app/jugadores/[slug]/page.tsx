...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${name} | FutStats`, ...(player.photoUrl != null ? { images: [player.photoUrl] } : {}) },
  };
}
...
export default async function PlayerPage({
  ...
  const isGoalkeeper = player.positions.some((position) => position.isPrimary && position.group === 'GK');
  const [data, content] = await Promise.all([
    getPlayerRecentMatches(player.id, isGoalkeeper),
      ...
      <section className="flex flex-wrap items-center gap-5 rounded-2xl border border-pitch-border bg-pitch-card p-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-pitch-muted">
            <span>{primaryPosition != null ? POSITION_LABEL[primaryPosition] ?? primaryPosition : 'Posición sin registrar'}</span>
            {player.currentTeam != null && (
              <>
                <span aria-hidden="true">·</span>
                <Link href={`/equipos/${player.currentTeam.slug}`} className="inline-flex items-center gap-1.5 text-pitch-accent hover:underline">
                  {player.currentTeam.crestUrl != null && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img width={18} height={18} src={player.currentTeam.crestUrl} alt="" className="h-[18px] w-[18px] object-contain" />
                  )}
                  {player.currentTeam.name}
                </Link>
              </>
            )}
            {player.shirtNumber != null && <><span aria-hidden="true">·</span><span>#{player.shirtNumber}</span></>}
          </p>
          <p className="mt-1 text-xs text-pitch-muted">
            {[player.nationality?.name, age != null ? `${age} años` : null, player.heightCm != null ? `${player.heightCm} cm` : null]
      ...
      {Object.keys(data.trends).length > 0 && (
        <section aria-label="Tendencias recientes" className="flex flex-wrap gap-2">
          {Object.entries(data.trends).map(([key, trend]) => (
            <TrendBadge key={key} direction={trend.direction} label={TREND_LABEL[key] ?? key} />
          ))}
        </section>
      )}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
          ...
          {mainMetrics.map((metric) => (
            <Stat
              ...
              sub={data.summary.metrics[metric]?.per90 != null ? `${data.summary.metrics[metric]!.per90}/90'` : undefined}
            />
          ))}
      ...
      <section>
              ...
              <article key={match.matchId} className="fs-panel flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3 text-sm">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate">
                    {match.isHome ? 'vs' : '@'} {match.rival} <span className="text-pitch-muted">({match.result})</span>
                  </p>
                  <p className="truncate text-xs text-pitch-muted">
                    {match.competition}{translatedRound != null ? ` · ${translatedRound}` : ''}
                  </p>
                </div>
                ...
                {isGoalkeeper ? (
                  <span className="text-xs">🧤 {match.saves ?? '—'} paradas · {match.goalsConceded ?? '—'} encajados</span>
                ) : (
                  <span className="text-xs">⚽ {match.goals ?? 0} · 🎯 {match.assists ?? 0}</span>
                )}
                {match.rating != null && (
                  <span className={`rounded px-2 py-0.5 text-xs font-semibold ${match.rating >= 7 ? 'bg-pitch-accent/15 text-pitch-accent' : match.rating < 6 ? 'bg-pitch-danger/15 text-pitch-danger' : 'bg-slate-500/15 text-slate-300'}`}>
        ...
        {data.benchOnly.length > 0 && (
          <p className="mt-3 text-xs text-pitch-muted">
            Convocado sin minutos en {data.benchOnly.length} partido(s) de este periodo; no cuentan en la ventana de rendimiento.
          </p>
        )}
      </section>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {content.news.map((item) => (
                  <li key={item.id} className="rounded-lg border border-pitch-border bg-pitch-card px-3 py-2">
                    <a href={item.url} rel="noopener noreferrer" target="_blank" className="hover:text-pitch-accent">{item.title}</a>
              ...
              <ul className="space-y-2 text-sm">
                {content.transfers.map((transfer) => (
                  <li key={transfer.id} className="rounded-lg border border-pitch-border bg-pitch-card px-3 py-2">
                    ...
                    <span className="mt-0.5 block text-xs text-pitch-muted">
                      {new Date(transfer.date).toLocaleDateString('es-ES')} · {transfer.fee ?? 'No revelado'} · Fuente: API-Football
                    </span>
                  </li>
                ))}
              </ul>
