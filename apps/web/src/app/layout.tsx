...
export const metadata: Metadata = {
  ...
  openGraph: {
    type: 'website', siteName: 'CornerMaximo', locale: 'es_ES',
  ...
  twitter: { card: 'summary_large_image', title: 'CornerMaximo | Sports Intelligence', description: 'Tu deporte. Tus datos. Tu ventaja.', images: ['/opengraph-image'] },
};
    ...
    <html lang="es">
      <body className="min-h-dvh pb-16 md:pb-0">
        <CspNonceProvider nonce={nonce}>
          ...
          <a href="#contenido" className="sr-only z-[120] rounded-xl bg-pitch-accent px-4 py-2 font-semibold text-white focus:not-sr-only focus:absolute focus:left-4 focus:top-4">Saltar al contenido principal</a>
          <header className="sticky top-0 z-30 border-b border-white/10 bg-[#05080d]/88 shadow-[0_12px_34px_-30px_rgba(0,0,0,.95)] backdrop-blur-2xl">
            <MainNav />
            <div aria-hidden="true" className="fs-rule absolute inset-x-0 bottom-0" />
          </header>
          <FavoriteHomeBanner />
          <main id="contenido" tabIndex={-1} className="mx-auto w-full max-w-[1480px] px-4 py-6 outline-none sm:px-6 lg:px-8 lg:py-8">
            {children}
          </main>
          <SiteFooter />
          <MobileBottomNav />
