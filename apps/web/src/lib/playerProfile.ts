...
import { getLastMatches, type LastMatchesResponse } from '@/lib/recent';
...
const cachedPlayerProfileCore = unstable_cache(queryPlayerProfileCore, ['player-profile-core'], {
  ...
  tags: [FOOTBALL_DATA_CACHE_TAG, 'player-profiles'],
});
...
async function queryPlayerRecentMatches(playerId: number, isGoalkeeper: boolean): Promise<LastMatchesResponse> {
  return getLastMatches(playerId, isGoalkeeper);
}

const cachedPlayerRecentMatches = unstable_cache(queryPlayerRecentMatches, ['player-recent-matches'], {
  revalidate: FOOTBALL_DATA_REVALIDATE_SECONDS,
  tags: [FOOTBALL_DATA_CACHE_TAG, 'player-profiles'],
});

export function getPlayerRecentMatches(playerId: number, isGoalkeeper: boolean): Promise<LastMatchesResponse> {
  return cachedPlayerRecentMatches(playerId, isGoalkeeper);
}
...
async function queryPlayerProfileContent(
  ...
  return {
    news: news.map((item) => ({ ...item, publishedAt: item.publishedAt.toISOString() })),
    transfers: transfers.map((item) => ({ ...item, date: item.date.toISOString() })),
  };
}

const cachedPlayerProfileContent = unstable_cache(queryPlayerProfileContent, ['player-profile-content'], {
  revalidate: PLAYER_CONTENT_REVALIDATE_SECONDS,
  tags: [FOOTBALL_DATA_CACHE_TAG, 'player-content'],
});
