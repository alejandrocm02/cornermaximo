...
interface RawTeamResponse {
  team: { id: number; name: string; code: string | null; founded: number | null; logo: string | null; country?: string };
  venue: { name: string | null; city: string | null; capacity: number | null } | null;
}

export function mapTeam(raw: RawTeamResponse, country: string): ProviderTeam {
  return {
